...
- **The RBAC tiers are covered by two role-local accounts, and they need an explicit password.** `meta/users.yml` declares `baooperator` (`roles: [operator]`) and `baoreader` (`roles: [reader]`), which the standard RBAC provisioning joins to `<group_root>/web-app-openbao/<role>` in Keycloak and to the matching LDAP role group. Each one's `password` resolves to a credential this role generates (`rbac_operator_password` / `rbac_reader_password` in `meta/secrets.yml`), **not** to the users fallback. That matters: a user with no explicit password inherits the `strong_password` fallback expression, which is re-evaluated on every render (`secrets.choice`, nothing persisted), so the value `svc-db-openldap` writes with `ldap_passwd` and the value `playwright.env.j2` renders can be different strings — and the LDAP bind then returns 403. The same applies to any role-declared account something has to authenticate as; `web-app-zammad` solves it the same way with `api_bot_password`. The expression falls back to the random default when OpenBao is not in the deployment, so the two accounts stay exactly as they were for every other role. `files/playwright/test-rbac-groups.js` logs each of the three accounts in through the LDAP auth method and asserts both the policy set it receives and what it can actually do: the operator writes an application secret and the reader is refused, and only the administrator reads back an ACL policy definition. `operator` and `reader` are declared by no other role's `meta/rbac.yml`, so the two accounts hold no privileges anywhere else.
