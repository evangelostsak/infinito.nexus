...
- **Every deploy rotates the credentials, and the role is built around that.** The platform regenerates generated-algorithm credentials on each deploy — that is how a changed Ansible password reaches the application instead of silently diverging from it. For OpenBao it means the `seal_key` and the AppRole `role_id`/`secret_id` a run is handed are *not* the ones the running instance was configured with, which would otherwise leave the node sealed (`initialized: true, sealed: true`) and the AppRole answering `invalid role or secret ID`. The role therefore carries the transition itself:
  - `tasks/06_rotate.yml` records what each deploy actually applied in `.applied.json` inside the compose project directory, `0600`, root-owned. The directory already holds the seal key in plaintext in `.env/env`, so this adds no new exposure class.
  - `tasks/00_core.yml` reads that file back **before** `sys-stk-backend` renders `env.j2` — the rendered env must already carry `OPENBAO_SEAL_KEY_PREVIOUS`, or the node comes up with no way to decrypt its own store.
  - `templates/openbao.hcl.j2` emits the `previous_key_id`/`previous_key` pair only while the two keys differ. The static seal supports `n-1` → `n` rotation, so the node unseals with the old key and rewraps to the new one.
  - `tasks/01_init.yml` attempts the AppRole login with the inventory credentials, falls back to the recorded ones, and fails loudly when neither is accepted. Whichever one was accepted is the SecretID OpenBao actually holds, so that is what `tasks/06_rotate.yml` destroys after registering the new one — rotation that only adds would revoke nothing.
  - `auth/approle/role/<role>/custom-secret-id` is **not** idempotent: writing a SecretID the role already holds answers `500 … SecretID is already registered`. The swap is therefore gated on the value having actually changed, which is also why the bootstrap in `tasks/02_auth.yml` pins directly instead of sharing the rotation path — at that point nothing is registered yet.
  - `current_key_id` is derived as a short hash of the key rather than a counter. Upstream requires the identifier to match its key; a derived id cannot drift out of step with one, and no deploy has to know which generation it is.
  The record is written last, after both pins, so a run that dies partway leaves the previous generation intact and the next deploy still authenticates. The one uncovered window is a failure *between* the two pins — the RoleID re-pinned, the SecretID not, or the reverse — which leaves a pairing neither the inventory nor the state file describes. Two generations is the whole window. Anything that skips a rotation puts the store outside it, and the instance is then recoverable only from its volume backup plus the matching seal key. The concrete way to do that is `MODE_RESET`: `sys-svc-compose/tasks/01_reset.yml` removes `DIR_COMPOSITIONS` wholesale while the named volumes survive, so the raft store outlives the record of the key that wraps it. Purge the entity's volumes too when you reset — the same asymmetry breaks every entity that keeps a credential inside its volume.
- **Other entities still desynchronise the same way, and those are not defects.** A surviving `mariadb_data` fails its healthcheck with `Access denied for user 'root'@'127.0.0.1'` for exactly the reason above, without a rotation path of its own. CI never sees it because every run starts clean; locally, run `make compose-entity-purge apps=<entity>` before redeploying.
- **A role-declared account cannot be logged in as unless it sets a password.** The tiers were first covered by two role-local accounts, `baooperator` and `baoreader`, which deployed and joined their RBAC groups correctly but returned 403 on every LDAP bind: a user declared without an explicit `password` inherits the `strong_password` fallback expression, which is re-evaluated on every render (`secrets.choice`, nothing persisted), so the value `svc-db-openldap` writes with `ldap_passwd` and the value `playwright.env.j2` renders can be different strings. Both accounts are gone now — the tiers are covered by moving `biber` through the groups instead — but the trap applies to any future role-declared account something has to authenticate as. Until [`f4398e1c5`](https://github.com/kevinveenbirkenbach/infinito-nexus-core/commit/f4398e1c58f3ce7d8d429472b2a023033242d747) is available here (it pins a password per declared user at inventory creation), such an account needs an explicit `password:`, as `user-administrator` and `web-app-semaphore` do.
