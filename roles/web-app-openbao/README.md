...
- **The RBAC tiers are covered by two dedicated accounts.** `meta/users.yml` declares `baooperator` (`roles: [operator]`) and `baoreader` (`roles: [reader]`), which the standard RBAC provisioning joins to `<group_root>/web-app-openbao/<role>` in Keycloak and to the matching LDAP role group. `files/playwright/test-rbac-groups.js` logs each of the three accounts in through the LDAP auth method and asserts both the policy set it receives and what it can actually do: the operator writes an application secret and the reader is refused, and only the administrator reads back an ACL policy definition. `operator` and `reader` are declared by no other role's `meta/rbac.yml`, so the two accounts hold no privileges anywhere else.
