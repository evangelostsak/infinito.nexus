...
- **The RBAC tiers are covered by two role-local accounts, and they need an explicit password.** `meta/users.yml` declares `baooperator` (`roles: [operator]`) and `baoreader` (`roles: [reader]`), which the standard RBAC provisioning joins to `<group_root>/web-app-openbao/<role>` in Keycloak and to the matching LDAP role group. Both set `password: "{{ ansible_become_password }}"`, as `user-administrator` and `web-app-semaphore` do. Leaving the field out does **not** work: a user without an explicit password inherits the `strong_password` fallback expression, which is re-evaluated on every render (`secrets.choice`, nothing persisted), so the value `svc-db-openldap` writes with `ldap_passwd` and the value `playwright.env.j2` renders can be different strings — and the LDAP bind then returns 403. This costs no extra exposure: `user-administrator` already gives the `administrator` account that same password, and that account holds the OpenBao *administrator* policy through the same LDAP group machinery, so these two are strictly less privileged than one that already shares the secret. `files/playwright/test-rbac-groups.js` logs each of the three accounts in through the LDAP auth method and asserts both the policy set it receives and what it can actually do: the operator writes an application secret and the reader is refused, and only the administrator reads back an ACL policy definition. `operator` and `reader` are declared by no other role's `meta/rbac.yml`, so the two accounts hold no privileges anywhere else.
