...
- **The RBAC tiers are covered by two test accounts, declared where `biber` is.** `inventories/development/default.yml` declares `baooperator` (`roles: [operator]`) and `baoreader` (`roles: [reader]`), which the standard RBAC provisioning joins to `<group_root>/web-app-openbao/<role>` in Keycloak and to the matching LDAP role group. They live in the CI inventory rather than in `meta/users.yml` for one concrete reason: a user with no explicit `password` falls back to `"{{ 42 | strong_password }}"`, and that expression is re-evaluated on every render (`secrets.choice`, nothing persisted), so the password written into LDAP and the password rendered into `playwright.env` are different strings and the login returns 403. Only an account with a literal password in the inventory can be logged in as. The same reasoning applies to any role-declared account that something needs to authenticate as. `files/playwright/test-rbac-groups.js` logs each of the three accounts in through the LDAP auth method and asserts both the policy set it receives and what it can actually do: the operator writes an application secret and the reader is refused, and only the administrator reads back an ACL policy definition. `operator` and `reader` are declared by no other role's `meta/rbac.yml`, so the two accounts hold no privileges anywhere else.
