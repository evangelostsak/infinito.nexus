...
- [x] A spec covers every RBAC tier, not only the denial. `meta/users.yml` declares `baooperator` and `baoreader`, whose `roles` lists put them in the operator and reader groups through the existing RBAC provisioning, and `test-rbac-groups.js` asserts for all three accounts that the policy set matches the tier and that the granted capabilities differ as the policies say: the operator writes an application secret where the reader gets 403, the reader still reads it back, and only the administrator reads an ACL policy definition. **Correction from implementation:** the two accounts first carried no `password`, which deployed them correctly but made every login return 403 — a user without one inherits the `strong_password` fallback, which `utils/templating/ansible.py` re-evaluates through `secrets.choice` on each render with nothing persisted, so `ldap_passwd` and `playwright.env` could receive different strings. The fix keeps the accounts role-local and gives each `password: "{{ ansible_become_password }}"`, matching `user-administrator` and `web-app-semaphore`. It adds no exposure, because `user-administrator` already assigns that password to the `administrator` account, which holds the OpenBao administrator policy through the same LDAP groups.
