...
- [x] A spec covers every RBAC tier, not only the denial. `inventories/development/default.yml` declares `baooperator` and `baoreader` next to `biber`, whose `roles` lists put them in the operator and reader groups through the existing RBAC provisioning, and `test-rbac-groups.js` asserts for all three accounts that the policy set matches the tier and that the granted capabilities differ as the policies say: the operator writes an application secret where the reader gets 403, the reader still reads it back, and only the administrator reads an ACL policy definition. **Correction from implementation:** the accounts were first declared in the role's `meta/users.yml`, which deployed them correctly but made every login return 403 — a user with no explicit `password` inherits `"{{ 42 | strong_password }}"`, which `utils/templating/ansible.py` re-evaluates through `secrets.choice` on each render with nothing persisted, so LDAP and `playwright.env` received different strings. Test accounts that must authenticate therefore belong in the inventory, where `biber` already lives.
