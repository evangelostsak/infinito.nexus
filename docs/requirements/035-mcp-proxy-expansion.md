...
- [`roles/test-e2e-cli/files/shared/mcp/contract.sh`](../../roles/test-e2e-cli/files/shared/mcp/contract.sh) proves the endpoint answers its declared contract, not that any particular client can route to it: it runs in a throwaway container joined to the *provider's* own network, because an `expose:`-only endpoint resolves nowhere else. Per-client routing is therefore still unproven by any test. Measured by hand on compose, Open WebUI resolves `baserow` to `192.168.206.4` and `homeassistant` to `192.168.79.2`, both distinct from the wildcard address, so the topology carries those two pairs; Swarm is unmeasured. [`isolation.sh`](../../roles/test-e2e-cli/files/shared/mcp/isolation.sh) does cover the adapter half, counting networks inside the sidecar for every `implementation: adapter` provider.
...
## n8n 1.100.1 Integration

`web-app-n8n` MUST be added as `direction: both` for the exact pinned 1.100.1 behavior: an SSE client and a Streamable HTTP server (`transport: streamable_http`, MCP Server Trigger typeVersion 2).
...
- publish its MCP endpoint only after activation and an authenticated MCP handshake;
- expose read-only operations first, with mutations in a separate opt-in variant and role.

The implementation MUST NOT claim that the newer instance-wide n8n administration MCP server exists in 1.100.1.
...
| `web-app-n8n` | native client and server candidate | Implement the SSE client and bearer-protected Streamable HTTP Trigger workflow contract in this requirement. Do not use features introduced only in n8n 2.13 or later. |
...
| `web-app-discourse` | Replace `hosted_only` with `stdio_only`. The audit of `discourse/discourse-mcp` at `v0.3.1` closed the sidecar path: ownership, licence, pinning and the read-only default all pass, but the HTTP transport serves one session per process, which upstream asserts itself in `src/test/transport.test.ts` at that tag. A shared multi-consumer endpoint cannot be built on it, and the deploy-time contract probe alone would leave the process in its `503` restart-required state. Enable nothing until upstream supports concurrent sessions. |
...
| `web-app-odoo` | Replace `hosted_only` with `unreviewed_third_party`. The review ran and selected no module. Odoo SA publishes no server and the OCA organisation holds no MCP repository, so the field is third-party only. The leading candidate, `ivnvxd/mcp-server-odoo` at `v0.8.0`, fails on maintenance breadth (197 of 231 commits from one author), on access control that ships as a separate Odoo Apps store module outside any pinnable tag, and on a mutating default tool scope. Re-review if Odoo SA or the OCA publishes a server. |
...
The section titles below are the classification vocabulary itself, and each role that ships a `meta/mcp.yml` is filed under the value that file declares. [test_mcp_audit_classification.py](../../tests/lint/repository/documentation/test_mcp_audit_classification.py) holds the two apart from drifting: it was written first and immediately named twenty-four roles whose audit entry contradicted their own metadata, nine of them shipping an adapter while the audit still called them untried candidates.
...
- [x] The operational metadata remains role-local, while the generated audit report includes deployed version, authoritative source, transport, authentication, real credential subject, read/write scope, tool allowlist, proposed implementation, selected consumers, and blocker. [audit.md](../contributing/design/role/services/mcp/audit.md) is rendered by [cli/build/docs/mcp_audit](../../cli/build/docs/mcp_audit/__main__.py) through `make mcp-audit`, which only reads `meta/mcp.yml` and never writes back into a role; [test_mcp_audit_report.py](../../tests/lint/repository/documentation/test_mcp_audit_report.py) imports the generator and fails when the committed report differs from a fresh run.
...
- [x] WordPress, Discourse, Odoo, Hermes, OpenClaw, Flowise, and the 11 missing upstream-path roles are reclassified exactly as required above before their old status is considered complete.
